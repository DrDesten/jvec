...
export class vec2 extends Float64Array {
     ...
     * @returns {vec2}
     */
    static new( object = 0, y ) {
        let tc_return
        tc_number( y )
        const vec = new vec2
        if ( typeof object === "number" )
            y === undefined
                ? ( vec[0] = object, vec[1] = object )
                : ( vec[0] = object, vec[1] = +y )
        else
    ...
    static fromArray( array, index = 0, stride = 1 ) {
        ...
        tc_number( index )
        tc_number( stride )
        tc_return = vec2.new( array[0 * stride + index], array[1 * stride + index] )
        tc_vec2( tc_return )
        return tc_return
    }
    ...
    static fromAngle( angle ) {
        let tc_return
        tc_number0( angle )
        tc_return = vec2.new( Math.cos( angle ), Math.sin( angle ) )
        tc_vec2( tc_return )
        return tc_return
    }
    ...
    static random() {
        let tc_return
        tc_return = vec2.new( Math.random(), Math.random() )
        tc_vec2( tc_return )
        return tc_return
    }
    ...
    static randomNorm() {
        let tc_return
        tc_return = vec2.new( randomNorm(), randomNorm() )
        tc_vec2( tc_return )
        return tc_return
    }
    ...
    static randomDir() {
        let tc_return
        tc_return = vec2.new( randomNorm(), randomNorm() ).normalize()
        tc_vec2( tc_return )
        return tc_return
    }
